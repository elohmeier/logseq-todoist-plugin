import type { Label, PersonalProject, Section, TodoistApi, WorkspaceProject } from "@doist/todoist-api-typescript";
...
export const fetchTodoistContext = async (
  ...
  if (requirements.labels) {
    const labels = await collectPaginatedResults<Label>((cursor) => api.getLabels(cursor ? { cursor } : undefined));
