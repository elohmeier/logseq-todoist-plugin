...
const makeContextRequirements = (
  ...
  const requiresProjects = (config?.groupBy === GroupingOption.Project
  ...
  const requiresLabels = config?.groupBy === GroupingOption.Labels
...
const fetchTasksByProject = async (api: TodoistApi, projectId: string): Promise<Task[]> => {
  return collectPaginatedResults<Task>((cursor) => api.getTasks({ projectId, cursor: cursor ?? undefined }));
};

const fetchTasksByFilter = async (api: TodoistApi, filter: string): Promise<Task[]> => {
  return collectPaginatedResults<Task>((cursor) => api.getTasksByFilter({ query: filter, cursor: cursor ?? null }));
};

const loadComments = async (api: TodoistApi, taskId: string): Promise<TaskAnnotations> => {
  const comments = await collectPaginatedResults((cursor) => api.getComments({ taskId, cursor: cursor ?? null }));
...
const buildDisplayTasks = async (
  ...
  return tasks.map((task, index): DisplayTask => {
    ...
    const creationDate = preferences.appendCreationDateProperty && task.addedAt
